...
shinyServer(function(input, output){
    output$prof1Res <- renderText({
        p1 <- res(input$Q.r.1, input$Q.h.1, input$t.i.1,
                  input$t.r.1, input$t.h.1, input$Threshold)
        paste("Profile 1 Resilience = ", round(p1, digits=2),
              "at Threshold = ", input$Threshold)
    })
    output$prof2Res <- renderText({
        p2 <- res(input$Q.r.2, input$Q.h.2, input$t.i.2,
                  input$t.r.2, input$t.h.2, input$Threshold)
        paste("Profile 2 Resilience = ", round(p2, digits=2),
              "at Threshold = ", input$Threshold)
    })
    output$ResPlot <- renderPlot({
        resilience <- resrat(input$Q.r.1, input$Q.r.2,
                             input$Q.h.1, input$Q.h.2,
    ...
    output$Profile1 <- renderPlot({
        ...
        profile1 <- buildProf(input$Q.r.1, input$Q.h.1, input$t.i.1,
                              input$t.r.1, input$t.h.1)
        m <- ggplot(profile1, aes(Time, Performance))
        m <- m +geom_path(color="red") + ylim(0, 1.5) + xlim(0, xAxis)
    ...
    output$Profile2 <- renderPlot({
        xAxis <- axLim(input$t.i.1, input$t.r.1, input$t.h.1,
                       input$t.i.2, input$t.r.2, input$t.h.2)
        profile2 <- buildProf(input$Q.r.2, input$Q.h.2, input$t.i.2,
                              input$t.r.2, input$t.h.2)
        p <- ggplot(profile2, aes(Time, Performance))
        p <- p +geom_path(color="green4")  + ylim(0, 1.5) + xlim(0, xAxis)
